...
public class Honda implements Engine, Brake, Ac{
    ...
    //Java does not support multiple inheritance for class parents.
    //If you make the parents interface, java support multiple inheritance
        ...
        1)Java does not support "multiple inheritance" for classes but if you put all common functionalities into a single parent class
          the parent class will be so huge which we do not want it because "atomic structure" is recommended in coding.
          atomic structure:to make every part as possible as small in coding
        2)To be able to use "multiple inheritance" in Java, Java created "interface" structure. You can make multiple interfaces parent of a class, so
          for interfaces "multiple inheritance" is supported in Java.
        3)"interface" is "interface", "interface" is not a class
        4)"interface"s have "common" and "mandatory" functionalities. "interface"s are "to-do list" for "concrete child classes".
    ...
    @Override           //every method in parent class must be override
    public void run() {
